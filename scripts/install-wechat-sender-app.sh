...
label="com.stardust.ceo-agent.wechat-sender"
source_app="${CEO_WECHAT_SENDER_OUTPUT_DIR:-${HOME}/Library/Caches/CEO Agent/WeChatSenderBuild/dist}/CEO WeChat Sender.app"
target_root="${HOME}/Applications"
target_app="${target_root}/CEO WeChat Sender.app"
...
staging_app="${target_root}/.CEO WeChat Sender.app.new"
backup_app="${target_root}/.CEO WeChat Sender.app.previous"
...
sender_executable="${target_app}/Contents/MacOS/CEO WeChat Sender"
/usr/libexec/PlistBuddy -c "Set :ProgramArguments:0 ${sender_executable}" "${target_plist}"
