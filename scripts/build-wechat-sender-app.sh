...
bundle_id="com.stardust.ceo-agent.wechat-sender"
app_name="CEO WeChat Sender"
build_home="${HOME}/Library/Caches/CEO Agent/WeChatSenderBuild"
output_dir="${CEO_WECHAT_SENDER_OUTPUT_DIR:-${build_home}/dist}"
build_root="${build_home}/work"
builder_python="${CEO_WECHAT_SENDER_BUILD_PYTHON:-${repo_root}/.venv-reader-build/bin/python}"
signing_identity="${CEO_WECHAT_SENDER_SIGNING_IDENTITY:-}"
adhoc=0
...
if [[ -z "${signing_identity}" && "${adhoc}" -ne 1 ]]; then
  printf '%s\n' \
    'A stable SIGNING_IDENTITY is required. Set CEO_WECHAT_SENDER_SIGNING_IDENTITY or pass --adhoc for a development-only build.' >&2
  exit 2
fi
...
pyinstaller_args+=("${repo_root}/app/wechat/sender_helper.py")
