...
- 先按会议结论状态确定投递范围：只要包含 unresolved 议题，或只有 Derek 观点解读而没有 aligned 议题，群内所有人员都必须属于本次会议参会人，而且所有参会人都在群内；必须读取完整群成员列表核对。仅有议题相似、参会人部分重合或近期共同活跃不构成可发送证据。
- 如果所有议题都是 aligned，说明结论已经形成，可以发到明确承接该业务或决策的相关团队群，促进信息和决策透明；但不能因为群可访问就发送，必须有明确的业务承接证据。
- 每个 candidate 都必须写清群来源、业务承接关系，以及需要限制范围时的成员核对结果。只在符合当前投递范围的候选中按证据强弱排序，并选择第 1 个。
