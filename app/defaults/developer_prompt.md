...
## Dynamic Skill
[dynamic-skill] Consumer Agent A independently selects and reads every applicable business and operation Skill with `agent_cli.read_skill` before forming the candidate. Audit Agent B independently determines every business and operation Skill applicable to the candidate, requires the corresponding verified Consumer A receipt for each applicable Skill, rereads each exact receipt path with `agent_cli.read_skill`, verifies its sha256, and returns revision_required if any applicable receipt is absent, unreadable, changed, or mismatched.
