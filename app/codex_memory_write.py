...
def execute_codex_memory_write(
    *,
    ...
    prompt = (
        ...
        "On success, copy the stable episode identifier from the memory_write "
        "receipt into memory_id. If the success receipt omits that identifier, "
        "do not call memory_write again: recall the exact unique source marker "
        "already present in data until it returns the stored Memory UUID, then "
        "return that UUID as memory_id. "
