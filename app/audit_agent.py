...
def _recovery_prompt(
    ...
    return (
        ...
        "absent, superseded, or ambiguous, and the exact result_digest from the matching live "
        "read receipt in this recovery turn. A target-scoped failed read may be "
        "cited only for ambiguous; it can never prove present or absent. Use present "
        "only when the read proves "
        "the old action happened, absent only when it proves the action did not "
        "happen, and ambiguous when human judgment is required. Use superseded "
        "only when a later message in the same target directly resolves the trigger "
        "or makes the proposed action obsolete; superseded requires that later-message "
        "read receipt and never permits a replay. Return outcome "
