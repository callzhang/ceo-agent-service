...
def _direct_delivery_ledger_absences(
    ...
    """Return ledger evidence only for direct deliveries absent for this trigger."""
    if store.has_sent_reply_for_trigger(task.conversation_id, task.trigger_message_id):
        return ()
...
def _recovery_prompt(
    ...
    if ledger_absences:
        ledger_guidance = (
            "\nThe service delivery ledger has already proved the following direct "
            "delivery actions absent for this exact trigger. Do not read or write them "
            "in this phase. Include one absent reconciliation entry for each using its "
            "exact supplied digest: "
            + json.dumps(
                [
                    {
                        "action_index": action_index,
