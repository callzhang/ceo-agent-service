...
def _process_claimed_work_summary_input(store, runner, work_input, *, dws=None) -> bool:
    ...
    try:
        ...
        if _is_runtime_outage_error(exc):
            ...
            # the provider is out. Wait without spending the item's retry
            # budget or recording a per-item Service error; the route pause
            # is the signal.
...
def _recover_orphaned_reply_tasks_on_service_start(settings: WorkerSettings) -> int:
    ...
    handed_off_completed_actions = (
        store.close_failed_reply_tasks_that_completed_an_external_action()
    )
    recovered_tasks = (
