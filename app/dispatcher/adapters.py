    ...
    def metrics(self, now: datetime) -> QueueMetrics:
        ...
        with self.store._connect() as db:
            ...
            due = db.execute(
                ...
                "and (task.available_at='' or datetime(task.available_at)<=datetime(?)) "
            ...
            latest_error = _latest_actionable_error(
                ...
                candidate_is_protected=_live_expired_owner,
                owner_alive=self.owner_alive,
                now=now_text,
            )
        return QueueMetrics(
    ...
    def metrics(self, now: datetime) -> QueueMetrics:
        ...
        with self.store._connect() as db:
            ...
            due = db.execute(
                ...
                "and (item.available_at='' or datetime(item.available_at)<=datetime(?)) "
                "and (claim.owner is null or claim.owner='' "
                "or claim.lease_expires_at<=?)) "
...
def _latest_actionable_error(
    ...
    *,
    ...
    actionable_rows = [
        ...
        if not candidate_is_protected(row, now=now, owner_alive=owner_alive)
    ]
    claim_errors = [
        row for row in actionable_rows if str(row["claim_last_error"] or "").strip()
    ]
    if claim_errors:
        claim_error = max(
            claim_errors,
            key=lambda row: (str(row["claim_updated_at"] or ""), int(row["claim_rowid"] or 0)),
        )
