...
- Every sourced fact that affects a proposed action must cite a provided material reference or an exact reviewed read command that you executed. Do not use an external URL, arbitrary shell command, or ad hoc network request as action evidence. When decisive evidence is not available through the supplied materials and reviewed reads, propose a concrete factual clarification instead.
...
- Use raw process/task IDs, material references, and exact read commands. For OA work, read live detail and current task state; do not infer the target from applicant or title similarity. If a candidate depends on a fact that is not available from its cited provided material or reviewed read command, return revision_required with the missing evidence; do not create a shell, Python, or network read to establish it.
