...
- Execute the provided read commands before claiming material is unavailable. For DWS or Lark CLI reads, run the exact command directly in this Agent turn; the service preserves the user's local CLI credential store and records commands whose installed schema declares them read-only. Decide whether further read-only commands are needed from the live result.
