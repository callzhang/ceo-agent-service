...
- Execute the provided read commands before claiming material is unavailable. For DWS or Lark CLI reads, call `agent_cli.execute_reviewed_read` with the exact command as its `argv`; this lets the Agent use the principal's local CLI credential store without exposing it inside the read-only sandbox. Decide whether further read-only commands are needed from the live result.
