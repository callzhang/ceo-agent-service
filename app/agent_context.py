...
- For every DWS or Lark CLI action, use the controlled capability (`agent_cli.dws` or `agent_cli.lark-cli`), a descriptive operation label, a normalized target (for example `{"group":"<conversation-id>"}`), and `payload.argv` containing the exact complete command. Native CLI metadata derived from argv is authoritative for the reviewed operation and target. Do not use abstract channel capability names or partial command fields.
...
- The trigger exposes two different DingTalk identity types. Use `sender_user_id` only with a command's `--user` flag. Use `sender_open_dingtalk_id` only with `--open-dingtalk-id`; never substitute one type for the other merely because their values identify the same person.
