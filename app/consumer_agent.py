...
def audit_developer_instructions(role_instruction: str) -> str:
    return _role_developer_instructions(
        ...
        capability_instructions=(
            ...
            "When a required reviewed capability is discovered through tool search, "
            "call it in this same turn; discovery is not an unavailable-tool result "
            "and is never a reason to return failed before attempting the required "
            "Skill and live reads. "
            ...
            "with `dws schema --cli-path \"<product> <command>\" --compact --format json` "
            "before composing the command. Copy the returned path "
