...
普通运行时不需要预先同步整个外部知识库。消息中出现钉钉在线文档、OA、日程、图片或文件材料时，worker 只把原始引用和精确读取命令交给 Consumer A；A 决定读取、展开和核对哪些材料，Audit B 在外部写入前独立复核。下载的本地材料通过 `agent_cli.execute_reviewed_read` 读取：DWS 和 Lark 使用发布的 effect 元数据；普通本地命令必须同时属于服务内置的可证明只读命令集，并通过主人 `~/.codex/config.toml` 中 `[ceo_agent.local_read_policy]` 的附加限制。任意 Python/shell、未知程序和具有写入参数的命令不会因为“未进黑名单”而放行；表格使用专用 `read_spreadsheet` 工具。没有策略或策略无效时，本地命令会拒绝执行。`agent_cli` 的每个受控工具都公开用途说明，确保 Agent 能按材料类型检索到日程、文档、文件等读取能力。受控重跑会启动新 Agent 会话，使新工具和新规则生效，不会恢复已失败会话中的旧工具调用；可重试但没有任何受控工具进展的 Consumer 回合也会清除旧会话后再试。读不到但能向对话参与者补齐的关键材料时，A 提出具体追问候选；不能猜测，也不要求 Derek 在“继续”与“追问”之间选择。
