...
- **会议结论 Memory 投递**：每个已发送的会议结论拥有一条独立的持久化投递记录。Worker 先以带过期时间的原子租约领取记录，再在有限并发内写入；长时间运行时，独立 SQLite 连接会定期续租。续租失败或租约已被回收的旧 Worker 不会写入终态，运行时已有的 generation 与幂等账本继续阻止重复写入。Status 会显示待处理、到期、超时、处理中、可重试与失败数量，以及最久等待、近一小时完成、真实活跃 Agent 和幽灵运行记录；到期超过 30 分钟或存在幽灵记录时，系统健康状态会降级。
...
微信投递把“发送动作明确未执行”和“动作已经开始但送达未知”分开保存。前者记录实际界面失败阶段，等待至少 5 分钟后最多自动重试两次；后者只读回消息记录后再决定，绝不因为重试而重复发送。已验证的最近会话即使预览被本地草稿替代，Sender 仍只点击唯一会话行并复核聊天标题；不会转为搜索或扩展联系人范围。
...
- 钉钉日程卡片可能在改期时原地更新而不产生新的消息 ID。慢路径会比较已处理卡片的最新可读内容；只有内容确实变化、日程仍有效且本人仍待响应时，才按同一消息业务对象的新输入版本重新触发 Consumer。普通已读消息仍按消息 ID 去重。
...
- Agent Cron scheduler：按任务自己的 Cron 和时区创建 trigger；停机不补跑，重叠轮次跳过，手动运行不移动计划。启动时按稳定 migration key 幂等 seed 十个默认任务：邮件、钉钉消息、钉钉日历邀请、每小时 `:30` 的近期消息恢复、会议、微信、OA、会议行动项、每周 OKR，以及每天 `20:00`（`Asia/Shanghai`）的 AI 听记同步。
- 服务命令任务：定时任务分两种执行形式。`command` 为空的任务由 Agent 按 prompt、Skill 和 Runtime 执行；`command` 非空的任务由 Dispatcher 在本进程内直接运行确定性发现代码，不创建 synthetic scheduled Agent 任务。当前十个种子任务全部是服务命令。服务命令发现真实对象后，才由对应的 reply、meeting、work-summary 或 Email classifier Consumer 处理；其可编辑 prompt 和精确 Skill 选择随真实输入保存，不回退加载全部业务 Skill。OKR 周报也保留为服务命令：它要用无头浏览器逐个读取管理者的实时 OKR 并在同一受控流程内完成分析，整轮可能远超普通 Agent 任务的空闲与总时长上限。
...
- Consumer Dispatcher：直接从 scheduled trigger/execution、reply、meeting、work summary、OKR 和 Todo outbox 的既有事实来源领取；内部唤醒、等待、租约和 recovery 没有用户设置。Dispatcher 面板的 `latest_error` 只反映当前可领取来源上的错误；终态失败请在 Queue、Attention 或历史中排查，reply 与 scheduled execution 分别按其 channel 显示。
...
- 邮件发现（“分类新邮件”任务）：`email-message-check-once` 每次只读取一轮已启用账户的未分类入口并创建幂等分类输入；扫描频率只由 Agent Cron 配置。Email classifier、Provider action 投递、Email Agent action consumer 和训练维护继续作为内部队列运行。分类输入保存实际 producing Trigger 的 run ID、任务 prompt 和精确 `ceo-email-classifier` Skill revision；同一稳定邮件被后续轮次再次发现时保留第一次输入与 Trigger 归属。
- 会议发现（“同步会议结论与管理者视角”任务）：读取 AI 听记与日历参会证据，只为 Derek 参会且明确结束至少 10 分钟的会议建队列；日历只用于确认参会名单。没有匹配日程时，逐字稿中识别到的说话者只能证明这些人发言过，不能证明会议是两人会议；没有触发条件的会议保持安静。每个已送达的钉钉会议对齐总结都会以稳定的会议 ID 入队并直接写入 Memory Connector。Memory 的正文只取实际发送的 `final_message`，不会附加本地 AI 听记摘要、转写或未发送的内部决策字段；写入队列与会议发送终态分离，失败按统一指数退避处理并在质量门和审计页展示。
- 会议对齐 consumer：先按讨论内容决定投递范围，再处理参会名单。客户、项目、产品、需求、交付、排期、测试、部署、客户沟通或跨团队行动均为业务内容：Agent 使用 DWS 搜索、排序并自行选择有明确业务承接关系的团队群；议题相似、参会人重合或近期活跃本身不构成投递证据。多个合理群时由 Agent 选择证据最强的群；权威会话信息证明所选群不可发送、且会议上下文已有稳定的日历组织者身份时，服务把同一总结私聊给该组织者，不按姓名搜索或猜测身份，也不回退到任意参会人。只有个人、非业务内容，且完整日历名单明确为 Derek 与另一位参会人时，才可以私信该另一位参会人。所选群不可发送时重试群验证、使用下一证据候选群，或使用稳定日历组织者 direct fallback；不回退为任意参会人私信。发送正文固定以 `【会议跟进】会议标题（会议时间）` 开头，便于收件人识别来源会议；真实 @ 默认限于参会人，非参会人只有会议转写明确说到是他的任务、由他负责、交给他确认或跟进时才 @。确认发送成功后复用 reply agent 的本地/Chrome notification 和钉钉会话点击跳转。dry-run 只分析到 `ready_to_send`，不会 claim 发送。
- `replay-recent-meetings` 会重新读取日历和听记证据，并只重开没有任何发送回执的 `no_action` 或 `failed` 会议任务；已发送或存在发送回执的任务保持终态，避免重复外发。
- 会议行动项与 Work Item：“将会议行动项整理到 Tasks”任务读取钉钉会议中新增或修改的行动项并创建工作摘要输入；Dispatcher 的 work summary consumer 核验证据后创建或更新需要持续跟进的 Tasks，到期 follow-up 由内部机制检查。
