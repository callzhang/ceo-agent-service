...
允许 reviewed MCP 的运行模式不设置全局 `tools.enabled_tools=[]`，否则 Codex 会把已配置 MCP 一并从模型工具列表中移除。运行时显式关闭 Codex 插件和 App，并按 MCP effect registry 为每个 server 生成精确的 `enabled_tools`；未登记的 server 被禁用。受控 CLI 的 stdio MCP 固定以服务仓库为 `cwd`，因此即使 Agent workspace 是独立的资料目录，也能加载服务模块。这样保留本机 Codex 登录、模型配置、Exa 和受控 DWS/Lark 通路，同时不暴露其他用户插件能力。完全无工具的独立只读任务继续使用全局空列表。
