...
- **飞书官方 Bot 通道（可选）**：通过官方 WebSocket Channel SDK 接收已批准用户的私聊和已批准群内结构化 `@机器人` 消息；默认关闭，发送默认需要人工确认。
...
- 飞书通道有独立总开关和 sender 开关；安装 SDK、填写 App ID 或只启用接收都不会自动开放真实发送。
...
- 可选：已认证的 Feishu/Lark CLI（默认二进制名为 `lark`），只用于材料读取和本机诊断
- 可选：`lark-channel-sdk` 与 `lark-oapi`，只在启用飞书官方 Bot 消息通道时安装；它们与本地 Feishu/Lark CLI 是两条独立能力
...
如需准备飞书 Bot 依赖，可安装可选依赖；安装本身不会创建应用、连接飞书或发送消息：
...
| `CEO_CODEX_PASSTHROUGH_MCP_SERVERS` | 允许显式透传给 agent 的 MCP 白名单；默认保留 `xiaoqing_interview,exa`。`memory_connector` 会优先复用本机 Codex MCP 配置；飞书材料读取/诊断走本地 CLI，飞书 Bot 消息通道走官方 Channel SDK，均不是默认 MCP |
| `CEO_FEISHU_CLI_BINARY` | 飞书只读/诊断 CLI 二进制名，默认 `lark`；CLI 写操作永久禁用 |
...
fallback。真实投递必须同时满足 `CEO_NOT_SEND_MESSAGE=0`、
`CEO_FEISHU_SENDER_ENABLED=1`；默认 `confirm` 模式还要求 delivery 逐条人工批准，
`auto` 只可作为另行授权的风险决策。可先用
`.venv/bin/ceo-agent feishu status` 做完全本地的脱敏检查，并在本机审计页
`/feishu/review` 审核目标和草稿。安装及 receive-only、凭证轮换、
故障处理和回滚步骤见 [飞书通道运维手册](docs/feishu-channel-operations.md)。
本地启动脚本仅在飞书总开关开启时检查 SDK、App ID 和 App Secret，并且只报告
`configured`/`missing`，不会输出凭证。

飞书回复判断使用独立的无工具 Codex 模式：忽略用户配置、只读 sandbox、关闭
Memory/DWS/所有 MCP、web、shell 和系统工具，并对任何意外 tool event 失败关闭。
当前不启用 Memory recall；这是无法硬证明 recall-only 全局工具白名单时的安全降级，
飞书判断只使用 prompt 中已归一化的当前会话上下文。
