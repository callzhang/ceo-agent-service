...
- **管理者 OKR 周报**：每周日读取 CEO-2 管理群成员的实时叮当 OKR、KR 评论/进展及可访问的独立渠道证据，按 `dingtang-okr-review` 对每个 KR 进行综合证据评分；业务 OKR、领导力、文化价值观分别计算并形成可审计的最终分。知识库主报告按管理会审阅场景呈现评分口径、重点进展、风险、决策项和下周检查点；逐人、逐 KR 评分文档作为主报告的下级节点发布（已有同名评分会自动迁移），主报告保留直达链接；群内只发送重点进展、风险和评分概览。
