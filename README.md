...
| `CEO_AGENT_RUNTIME_ROUTES` / `CEO_FRIDAY_RUNTIME_BASE_URL` / `CEO_FRIDAY_RUNTIME_PROJECT_ID` / `CEO_FRIDAY_RUNTIME_MODEL` / `CEO_FRIDAY_RUNTIME_TICKET`（或 `CEO_FRIDAY_SESSION_TOKEN`） | 可选 Friday Runtime fallback：启用 `friday_runtime` 后填写 Friday Runtime 地址、已存在的 project ID、模型和一个 Runtime 凭据。Friday 使用已有的 `CEO_CODEX_API_BASE_URL` 与 `CEO_CODEX_API_KEY` 作为 provider 配置（映射为 `FRIDAY_LLM_BASE_URL` / `FRIDAY_LLM_API_KEY`），不再新增或复制 MiniMax URL/key；Runtime ticket/session token 仍只用于访问 Friday Runtime。本地无鉴权测试必须显式设置 `CEO_FRIDAY_RUNTIME_AUTH_DISABLED=1`。 |
