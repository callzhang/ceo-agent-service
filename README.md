...
- **会议结论 Memory 投递**：每个已发送的会议结论拥有一条独立的持久化投递记录。Worker 先以带过期时间的原子租约领取记录，再在有限并发内写入；长时间运行时，独立 SQLite 连接会定期续租。续租失败或租约已被回收的旧 Worker 不会写入终态，运行时已有的 generation 与幂等账本继续阻止重复写入。
