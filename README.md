...
- 可选：`lark-channel-sdk` 与 `lark-oapi`，只在启用飞书官方 Bot 消息通道时安装；它们与读取飞书文档的 `lark-cli` 是两条独立能力
...
| `CEO_CODEX_PASSTHROUGH_MCP_SERVERS` | 允许显式透传给 agent 的 MCP 白名单；默认保留 `xiaoqing_interview,exa`。`memory_connector` 会优先复用本机 Codex MCP 配置；飞书文档走 `lark-cli`，飞书 Bot 消息通道走官方 Channel SDK，二者都不是默认 MCP |
