...
- `/email`：Email 控制台展示保存邮件的可读正文。HTML 邮件只投影可见文字；样式、脚本和远程资源不会在 Console 中执行或加载，原始 MIME 内容仍保留在本地邮件存储中。
...
- 会议发现（“同步会议结论与管理者视角”任务）：读取 AI 听记与日历参会证据，只为 Derek 参会且明确结束至少 10 分钟的会议建队列；日历只用于确认参会名单。没有匹配日程时，逐字稿中识别到的说话者只能证明这些人发言过，不能证明会议是两人会议；没有触发条件的会议保持安静。每轮还会把每个已送达会议的结论、管理者观点和可用 AI 听记摘要合并为一条 `memory_write` JSONL 记录，避免把同一会议的原始听记和会后结论分别导入长期记忆。
