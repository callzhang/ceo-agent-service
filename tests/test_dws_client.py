...
def test_read_unread_messages_accepts_direct_result_rows(monkeypatch):
    monkeypatch.setattr(dws_client, "_local_time_zone", lambda: TEST_LOCAL_TZ)
    payload = {
        "result": [
            {
...
def test_read_unread_messages_accepts_deeply_nested_message_rows(monkeypatch):
    monkeypatch.setattr(dws_client, "_local_time_zone", lambda: TEST_LOCAL_TZ)
    payload = {
        "result": {
            "data": {
...
def test_read_unread_messages_falls_back_to_recent_conversation_window(monkeypatch):
    monkeypatch.setattr(dws_client, "_local_time_zone", lambda: TEST_LOCAL_TZ)
    unread_payload = {"result": {"value": {"unexpected": "envelope"}}}
    recent_payload = {
        "result": {
            "messages": [
                {
    ...
    client = SequenceRecordingDwsClient([unread_payload, recent_payload])
    conversation = DingTalkConversation(
        open_conversation_id="cid-1",
        title="Friday",
        single_chat=False,
...
@pytest.mark.parametrize("alias", ("@磊哥", "@所有人", "@all"))
def test_read_broadcast_messages_preserves_configured_alias(alias: str):
    client = RecordingDwsClient({"messages": []})

    assert client.read_broadcast_messages((alias,), limit=10, lookback_hours=1) == []
    search_commands = [command for command in client.commands if "search" in command]

    assert len(search_commands) == 1
    assert search_commands[0][search_commands[0].index("--query") + 1] == alias
...
def test_the_push_title_of_a_markdown_message_has_no_markdown_markers():
    client = DwsClient(dws_bin="dws")
