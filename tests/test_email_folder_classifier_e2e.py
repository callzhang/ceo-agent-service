...
def _maturity(model_id: str) -> CandidateMaturityEvidence:
    ...
    eligible = HistoricalEligibility(0.97, 24 + snapshot_number, 11 + snapshot_number)
...
def test_warmed_production_runtime_cached_path_includes_full_local_pipeline(tmp_path):
    ...
    assert (
        store.classifier_runtime_observability(model_id=second.model_id)["timing"][
            "warm_success_cache"
        ]["sample_count"]
        == 100
    )
...
def test_component_chain_folder_classifier_lifecycle_uses_sequential_fallback_and_no_reply():
    ...
    fallback = route_online_classification(
        ...
        enqueue_agent=lambda _current: calls.append("agent-fallback") or "agent-result",
    ...
    entries = extract_unsubscribe_entries(
        body_html=(
            '<a href="https://newsletter.example/unsubscribe?id=e2e">unsubscribe</a>'
        )
    )
...
def test_real_store_training_registry_runtime_and_historical_action_integration(
    ...
    assert (
        first.maturity.source_snapshot_digest != second.maturity.source_snapshot_digest
    )
    ...
    class Source:
        def fetch_uid_batch(self, *_args, **_kwargs):
            return type(
                "Batch", (), {"uidvalidity": 1, "messages": (provider_message,)}
            )()
    ...
    assert (
        store.get_provider_classification_state(int(accepted.persisted["id"]))["state"]
        == "junk"
    )
    ...
    def locate(_hint):
        ...
        return ProviderMessageState(
            ...
            important_signal_names=frozenset(
                flag for flag in state["flags"] if flag in {"\\Flagged", "$Important"}
            ),
            required_important_signal_names=frozenset({"\\Flagged"}),
            locator=current,
        )
...
def test_live_mailbox_move_flag_truth_and_restore_is_opt_in(tmp_path):
    ...
    def read_complete_flags(hint):
        provider = connect()
        try:
            state = provider.read_state(hint, action_type=EmailAction.FLAG_IMPORTANT)
    ...
    def raw_provider_move(hint, target_folder):
        ...
        try:
            ...
            assert state.locator is not None
            actual = state.locator
            assert target_folder in {
                item.display_name for item in provider.list_folders()
            }
            ...
            provider._select(actual.folder, readonly=False)
            command = "MOVE" if "MOVE" in provider._capabilities() else "COPY"
            status, _ = provider.session.uid(
                command,
                str(actual.uid),
...
def test_live_cached_and_gpu_latency_budgets_are_opt_in(tmp_path):
    ...
    for seconds in (0, 1):
        ...
        evidence = {
            ...
            "source_snapshot_observed_at": (
                datetime(2026, 9, 8, tzinfo=timezone.utc) + timedelta(seconds=seconds)
            ).isoformat(),
