...
def test_owner_can_record_the_historical_systematic_error_review(tmp_path: Path):
    """Promotion waits on this review, and nothing but the owner writes it."""

    database = tmp_path / "historical-review.sqlite3"
    registry = EmailModelRegistry(tmp_path / "models")
    service = SimpleNamespace(
        registry=registry,
        retrain_state_path=tmp_path / "models" / "retrain-state.json",
    ...
    with TestClient(app) as client:
        before = client.get("/api/console/email/learning")
        reviewed = client.post(
            "/api/console/email/learning/historical-review",
            json={"resolved": True, "note": "错标审核已完成"},
        )
        invalid = client.post(
            "/api/console/email/learning/historical-review",
            json={"resolved": "yes"},
        )
        after = client.get("/api/console/email/learning")

    assert before.json()["learning"]["historical_review"]["unresolved"] is True
    assert before.json()["learning"]["historical_review"]["source"] == "state_missing"
    assert reviewed.status_code == 200
    assert invalid.status_code == 400
    review = after.json()["learning"]["historical_review"]
    assert review["unresolved"] is False
    assert review["source"] == "owner_review"
    assert review["reason"] == "错标审核已完成"
    assert registry.historical_systematic_error_state().unresolved is False
