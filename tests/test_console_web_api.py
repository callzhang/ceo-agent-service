...
def test_console_tutorial_run_persists_redacted_wechat_summaries(
    tmp_path: Path, monkeypatch
):
...
def test_console_agent_runtime_save_without_routes_is_refused(
    ...
    """This payload answered 200 已保存 and cut the routes to codex_oauth."""
...
def test_console_history_puts_what_waits_on_derek_first(tmp_path: Path):
    """An item handed to Derek must not slide down the page.

    Sorted by time alone it does, as ordinary work lands above it -- the
    opposite of what the status means. It is the only status that needs him
    to read it and the only one that never resolves itself. On 2026-09-19 two
    tasks were handed over and History listed neither anywhere near the top.
    """
    store = AutoReplyStore(tmp_path / "worker.sqlite3")
    store.record_reply_attempt(
        conversation_id="waiting-conversation",
        conversation_title="Waiting on Derek",
        trigger_message_id="waiting-message",
        trigger_sender="Derek",
        trigger_text="decide this",
        action="chat_message",
        sensitivity_kind="",
        codex_reason="test",
        draft_reply_text="already sent by hand",
        send_status="needs_human",
    )
    for index in range(3):
        store.record_reply_attempt(
            conversation_id=f"later-{index}",
            conversation_title=f"Ordinary work {index}",
            trigger_message_id=f"later-message-{index}",
            trigger_sender="Derek",
            trigger_text="routine",
            action="chat_message",
            sensitivity_kind="",
            codex_reason="test",
