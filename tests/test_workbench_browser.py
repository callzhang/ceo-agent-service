...
def test_mobile_conversation_fills_the_available_workbench_height(tmp_path: Path):
    sync_api = pytest.importorskip("playwright.sync_api")
    project_root = Path(__file__).resolve().parents[1]
    asset_dir = project_root / "app" / "static" / "workbench"
    if not (asset_dir / "index.html").is_file():
    ...
    task = store.create_task(title="Mobile conversation", runtime_kind="codex")
    turn = store.create_turn(
        task.id,
        user_text="Prepare the operating review",
        client_request_id="mobile-conversation",
    )
    assert store.claim_next_turn(owner="browser-test") is not None
    store.complete_turn(
        turn.id,
        status=TurnStatus.COMPLETED,
        final_text="Operating review ready.",
        owner="browser-test",
    )
    ...
    try:
        with sync_api.sync_playwright() as playwright:
            ...
            try:
                ...
                page.goto(
                    f"http://127.0.0.1:{port}/?task={task.id}",
                    wait_until="networkidle",
                )
                page.locator(".conversation-workspace").wait_for(state="visible")
                ...
                assert dimensions["workspaceHeight"] == pytest.approx(
                    dimensions["panelHeight"], abs=1
                )
                assert dimensions["timelineHeight"] >= 300
                assert dimensions["composerBottom"] == pytest.approx(
                    dimensions["panelBottom"], abs=1
                )
            finally:
                browser.close()
    finally:
