...
def configured_embedding_training(monkeypatch):
    monkeypatch.setenv(
        "CEO_EMAIL_EMBEDDING_URL", "https://embedding.example/v1/embeddings"
    )
    monkeypatch.setenv("CEO_EMAIL_EMBEDDING_DIMENSION", "768")
    monkeypatch.setenv("CEO_EMAIL_EMBEDDING_REVISION", "test-r1")
        ...
        return SimpleNamespace(
            ...
            request_manual_training=lambda **_: SimpleNamespace(
                decision=SimpleNamespace(
                    due=False, reason="training_selection_recorded", pending_examples=0
                ),
                training_run=None,
            ),
        )
...
def test_embedding_training_is_not_advertised_as_configured_without_endpoint(
    tmp_path, monkeypatch
):
    monkeypatch.delenv("CEO_EMAIL_EMBEDDING_URL")
    client, _store, _registry = client_for(tmp_path)

    learning = client.get("/api/console/email/learning").json()["learning"]
    family = next(
        item for item in learning["model_families"] if item["family"] == "embedding-mlp"
    )

    assert family["supported"] is True
    assert family["configured"] is False
    assert "CEO_EMAIL_EMBEDDING_URL" in family["reason"]


def test_learning_exposes_training_source_provenance_and_selection_is_executable(
    tmp_path, monkeypatch
):
    client, store, registry = client_for(tmp_path)
    monkeypatch.setattr(
        store,
        "list_selected_training_records",
        lambda: [
            {
                "source": "user_feedback",
                "stable_message_identity": "user-1",
                "category_key": "work",
                "account_id": "a",
                "normalized_model_input": "one",
            },
            {
                "source": "user_feedback",
                "stable_message_identity": "user-2",
                "category_key": "work",
                "account_id": "a",
                "normalized_model_input": "two",
            },
            {
                "source": "agent_auto_label",
                "stable_message_identity": "agent-1",
                "category_key": "legal",
                "account_id": "a",
                "normalized_model_input": "three",
            },
        ],
    )
    monkeypatch.setattr(
        ...
        lambda: {
            ...
            "sample_count": 3,
            "group_count": 2,
            "category_sample_counts": {"work": 2, "legal": 1},
        },
    )
    monkeypatch.setattr(
        store,
    ...
    assert (
        next(
            row["sample_count"]
            for row in rows
            if row["source"] == "user_feedback" and row["category"] == "work"
        )
        == 2
    )
    assert (
        next(
            row["unique_trainable_count"]
            for row in rows
            if row["source"] == "agent_auto_label"
        )
        == 1
    )
...
def test_training_preview_marks_selected_sources_as_frozen_on_submission(
    tmp_path, monkeypatch
):
...
def test_learning_exposes_model_family_support_and_persists_selection(
    tmp_path, monkeypatch
):
    client, store, registry = client_for(tmp_path)
    monkeypatch.setattr(
    ...
    assert response.status_code == 202
    assert captured["selection"]["model_families"] == [
        "embedding-mlp",
        "fasttext",
        "tfidf-logistic-regression",
    ]
    assert response.json()["learning"]["selection"]["model_families"] == [
        "embedding-mlp",
        "fasttext",
        "tfidf-logistic-regression",
    ]
...
def test_training_selection_rejects_unknown_values_and_malformed_json(
    tmp_path, monkeypatch
):
    client, store, _registry = client_for(tmp_path)
    monkeypatch.setattr(
...
def test_legacy_nonempty_training_selection_defaults_model_family(
    tmp_path, monkeypatch
):
    client, store, _registry = client_for(tmp_path)
    monkeypatch.setattr(
...
def test_promotion_config_changes_do_not_activate_model(tmp_path):
    ...
    assert (
        client.put("/api/console/email/promotion-config", json=payload).status_code
        == 409
    )
...
def test_primary_switch_rejects_unmeasured_candidate(tmp_path):
    client, store, registry = client_for(tmp_path)
    response = client.put(
        "/api/console/email/runtime-mode",
        json={
            "mode": "model_primary",
            "model_id": "no-candidate",
            "request_id": "r1",
...
def test_category_update_rejects_stale_version_before_provider_calls(tmp_path):
    ...
    response = client.put(
        ...
        json={
            ...
            "threshold": 0.95,
            "enabled": False,
            "description_version": "new-description",
...
def ready_client(tmp_path, monkeypatch):
    ...
    current["compatibility"]["description_version"] = _active_description_set_version(
        store
    )
    current["metrics"]["categories"]["legal"].update(
        precision=0.98, recall=0.98, f1=0.98, support=25
    )
    ...
    previous["source_snapshot_id"] = (
        "email-folder-snapshot-20260908T070000.000000Z-cccccccccccc"
    )
    ...
    for metric in [
        previous["metrics"]["categories"]["legal"],
        previous["metrics"]["important"],
    ]:
    ...
    for evidence in (previous, current):
        (registry.staged_evidence / (evidence["model_id"] + ".json")).write_text(
            json.dumps(evidence)
        )
        (
            registry.embedding_artifacts / (evidence["model_id"] + ".artifact")
    ...
    monkeypatch.setattr(
        runtime,
        "_prepare_online_activation",
        lambda registry, model_id, **_: prepare(
            registry, model_id, classifier_loader=lambda _: model
        ),
    )
...
def test_ready_candidate_switch_and_disable_roundtrip(tmp_path, monkeypatch):
    ...
    assert (
        client.put("/api/console/email/runtime-mode", json=request).status_code == 200
    )
    response = client.put(
        "/api/console/email/runtime-mode",
        json={
            ...
            "expected_mode": "model_primary",
            "expected_model_id": current["model_id"],
        },
    )
    assert response.status_code == 200, response.text
...
def test_active_model_with_unreadable_staged_file_keeps_learning_available(
    tmp_path, monkeypatch
):
    client, _, registry, current = ready_client(tmp_path, monkeypatch)
    assert (
        client.put(
            "/api/console/email/runtime-mode", json=enable_request(current)
        ).status_code
        == 200
    )
    ...
    assert learning["promotion_gate"]["promotion_eligible"] is False
    assert any(
        row["model_id"] == current["model_id"] for row in learning["staged_models"]
    )
...
def test_learning_keeps_model_rows_when_candidate_artifact_is_unreadable(
    tmp_path, monkeypatch
):
...
def test_learning_preserves_healthy_rows_with_malformed_metric_containers(
    ...
    for key in path[:-1]:
        target = target[key]
    target[path[-1]] = value
    (registry.staged_evidence / f"{current['model_id']}.json").write_text(
        json.dumps(corrupt)
    )
    response = client.get("/api/console/email/learning")
    assert response.status_code == 200
    learning = response.json()["learning"]
    assert [row["model_id"] for row in learning["staged_models"]] == [
        current["compatibility"]["parent_model_id"]
    ]
    assert learning["registry_issues"]
    assert learning["promotion_gate"]["promotion_eligible"] is False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "mode": "agent_primary",
            "promotion_config_version": "v1",
            "mode_transitions": [],
        },
    ],
)
def test_learning_reports_corrupt_control_manifest_and_switch_preserves_it(
    tmp_path, payload
):
    client, store, registry = client_for(tmp_path)
    ...
    learning = response.json()["learning"]
    assert any(
        issue["integrity_error"] == "online_control_invalid"
        for issue in learning["registry_issues"]
    )
...
def test_category_versions_are_generated_and_history_preserves_descriptions(tmp_path):
    ...
    register_email_routes(
        app, lambda: store, folder_binding_coordinator=_VerifiedFolderCoordinator()
    )
    client = TestClient(app)
    ...
    update.update(
        core_description="Updated partner criteria",
        expected_current_version=first["config_version"],
    )
    ...
    assert (
        client.put("/api/console/email/config/partner_updates", json=update).status_code
        == 409
    )
    response = client.get("/api/console/email/config/partner_updates/history")
    assert response.status_code == 200
    rows = response.json()["items"]
    assert [row["config_version"] for row in rows] == [
        second["config_version"],
        first["config_version"],
    ]
    assert [row["config"]["core_description"] for row in rows] == [
        second["core_description"],
        first["core_description"],
    ]
    assert all(
        row["description_version"] == row["config"]["description_version"]
        for row in rows
    )
    assert all(row["revision_id"] and row["created_at"] for row in rows)
    assert (
        client.get("/api/console/email/config/unknown_category/history").status_code
        == 404
    )


def test_category_history_projects_only_description_configuration(
    tmp_path, monkeypatch
):
    client, store, registry = client_for(tmp_path)
    ...
    monkeypatch.setattr(
        store, "list_category_description_revisions", lambda _: revisions
    )
