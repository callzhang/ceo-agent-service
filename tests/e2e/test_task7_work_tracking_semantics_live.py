...
class SemanticCase:
    ...
    verify: Callable[[TaskAgentDecision], None]
...
def _verify_discard(decision: TaskAgentDecision) -> None:
    assert decision.action == "discard"
    assert decision.todo_changes == []
    assert decision.follow_up_drafts == []


def _verify_owned_todo(decision: TaskAgentDecision) -> None:
    creates = _todo_changes(decision, "create")
    assert creates
    assert creates[0].owner_user_id == "uid-alex"
    assert creates[0].owner_evidence


def _verify_bound_follow_up(decision: TaskAgentDecision) -> None:
    assert decision.follow_up_drafts
    assert all(draft.todo_id or draft.todo_ref for draft in decision.follow_up_drafts)
    created_refs = {
        change.todo_ref
    ...
    assert all(
        draft.todo_id or draft.todo_ref in created_refs
        for draft in decision.follow_up_drafts
    )


def _verify_speaker_not_owner(decision: TaskAgentDecision) -> None:
    owners = [change.owner_user_id for change in decision.todo_changes]
    owners.extend(draft.owner_user_id for draft in decision.follow_up_drafts)
    assert "uid-speaker" not in owners


def _verify_open_todo_follow_up(decision: TaskAgentDecision) -> None:
...
def _verify_completed_suppression(decision: TaskAgentDecision) -> None:
...
def _verify_owner_correction(decision: TaskAgentDecision) -> None:
...
def _verify_reply_updates_existing(decision: TaskAgentDecision) -> None:
    assert _todo_changes(decision, "create") == []
    assert decision.follow_up_drafts == []
    assert any(change.todo_id == 42 for change in decision.todo_changes)


def _verify_stale_suppression(decision: TaskAgentDecision) -> None:
    changes = _follow_up_changes(decision, "suppress", "close")
    assert changes and changes[0].follow_up_id == 8
    assert decision.follow_up_drafts == []


def _verify_sensitive_direct_target(decision: TaskAgentDecision) -> None:
    assert decision.follow_up_drafts
...
CASES = (
    ...
    SemanticCase(
        "important_commitment_creates_todo_with_owner_evidence",
        {
            ...
            "identity": {"name": "Alex", "user_id": "uid-alex", "verified": True},
            "owner_evidence": "Alex said: I own this and will deliver it by Friday.",
        },
        _verify_owned_todo,
    ),
    SemanticCase(
