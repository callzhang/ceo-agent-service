...
def test_current_schema_removes_legacy_agent_runs_tool_events_column(
    tmp_path: Path,
):
...
def test_reconcile_confirmation_required_run_stays_failed(
    ...
    store = AutoReplyStore(tmp_path / "worker.sqlite3")
    store.enqueue_reply_task(
        conversation_id="cid-legacy-confirmation",
        conversation_title="Management group",
        single_chat=False,
        trigger_message_id="msg-legacy-confirmation",
        trigger_create_time="2026-09-15 10:00:00",
        trigger_sender="Derek",
        trigger_text="Please confirm the reviewed action.",
    )
    [task] = store.claim_reply_tasks(limit=1)
    run = _claim_audit_run(
        store, task.id, task.execution_generation, owner="audit"
    ...
    with store._connect() as db:
        db.executescript(
            ...
            create trigger reject_actionable_resolution before update on reply_attempts
                when old.id={source_id} and new.send_status='pending'
            begin
...
def test_stale_processing_task_ignores_future_lease_without_recent_heartbeat(
    tmp_path: Path,
) -> None:
    store = AutoReplyStore(tmp_path / "worker.sqlite3")
    task_id = _enqueue_universal_reply_task(store)
    task = store.get_reply_task(task_id)
    assert task is not None
    _claim_audit_run(store, task.id, task.execution_generation, owner="stale-worker")

    with sqlite3.connect(store.path) as db:
        db.execute(
            "update reply_tasks set locked_at='2026-07-20 10:00:00', updated_at='2026-07-20 10:00:00' where id=?",
            (task.id,),
        )
        db.execute(
            "update agent_runs set updated_at='2026-07-20 10:00:00', lease_expires_at='2099-01-01 00:00:00' where reply_task_id=?",
            (task.id,),
        )

    stale = store.list_stale_processing_reply_tasks(60)
...
def test_current_schema_reopens_and_repairs_old_runtime_attempt_execution_shape(
    ...
    assert store_module.STORE_SCHEMA_VERSION == "2026-09-16.1"
    assert {
        "lease_owner",
        "lease_expires_at",
        "attempt_purpose",
...
def test_the_attempt_says_what_the_task_says_after_a_hand_over(tmp_path: Path):
    """Two projections of one piece of work, and only one was updated.

    Derek opened /attempts/9670 and saw a failure for work already handed to
    him, and History listed no needs_human at all while four tasks sat in it.
    """
    store = AutoReplyStore(tmp_path / "attempt-projection.sqlite3")
    store.enqueue_reply_task(
        conversation_id="cid-1", conversation_title="Group", single_chat=False,
        trigger_message_id="msg-1", trigger_create_time="2026-09-19 10:00:00",
        trigger_sender="Derek", trigger_text="handle this",
    ...
    with store._connect() as db:
        db.execute(
            "update reply_attempts set agent_run_id=?", (claim.run.id,)
        )
