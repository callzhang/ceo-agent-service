...
def test_codex_session_lock_replaces_stale_lock(tmp_path):
    ...
    with store._connect() as db:
        db.execute(
            ...
            set locked_at=datetime('now', '-31 minutes')
...
def test_codex_session_lock_outlives_direct_agent_total_timeout(tmp_path):
    store = AutoReplyStore(tmp_path / "worker.sqlite3")
    ...
    assert store.acquire_codex_session_lock("cid-1", "reply:msg-2") is False
