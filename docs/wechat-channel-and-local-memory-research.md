...
- **当前决定：** 暂不执行任何改动客户端或系统的步骤，优先寻找/评估第 1 类零风险方法；工具链与完整步骤已备好，随时可在你确认后启用。
...
也就是说，`openclaw-weixin` 解决的是**独立 Bot 身份**在 Bot 会话中的收发；本地数据库读取解决的是个人主号已落盘历史的读取。二者组合可以实现“读主号历史 + 用 Bot 对外沟通”，但**尚未解决个人微信主号身份的自动发送**。
...
- 会修改 `/Applications/WeChat.app`（去 Hardened Runtime，ad-hoc 签名）：可通过还原备份或重装恢复。
- 需要 `sudo`，且需你本人完成两次扫码（登录、退出后重登录）。
...
5. **重签名副本**：沙盒容器/会话被换成新的，读不到原账号数据（本文早前已踩坑，根因已定位）。
...
2026-07-21 在本机 ad-hoc 构建中已验证：独立 Mach-O 进程成功启动；主服务经 Socket
...
4. **Bot 身份测试**：用第二个测试微信号与 iLink Bot 对话，验证收消息、带 `context_token` 回复、接收端展示的 Bot 身份，以及是否无法读取测试号既有历史。
5. **分阶段集成**：本地历史只用于上下文与经验候选；Bot 通道仅用于独立 Bot 对话。个人主号自动发信不在本方案内。
...
- 如果未来腾讯提供个人微信主号的正式、授权 API，应重新评估；在此之前，不以逆向或重签名方式实现主号自动发信。
