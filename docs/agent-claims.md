...
| codex-attempt-projection-repair | app/store.py, app/audit_web.py, tests/test_store.py, docs/architecture.md | Collapse legacy duplicate `reply_attempt` projections for one reply task into its current attempt while retaining all agent-run history; prior broad owner is idle and these target files have no uncommitted diff | 2026-09-15 |
