...
| Codex session `scheduled-dead-owner-takeover` | `app/dispatcher/adapters.py`, `tests/test_consumer_dispatcher.py`, `CHANGELOG.md` | immediately reclaim scheduled Trigger leases whose recorded owner PID no longer exists while preserving generation fencing | 2026-09-14 |
