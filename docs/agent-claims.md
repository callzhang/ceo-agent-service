...
| Codex session `calendar-card-revision-recovery` | `app/worker.py`, `app/store.py`, `app/agent_cron/seeds.py`, `tests/test_worker.py`, `tests/test_store.py`, `tests/test_workbench_store.py`, `tests/test_agent_cron_seeds.py`, `tests/test_scheduled_task_service_regressions.py`, `README.md`, `CHANGELOG.md`, `docs/architecture.md`, `docs/runtime-mechanism.md`, `docs/superpowers/specs/2026-09-08-agent-cron-and-managed-minutes-skill-design.md` | requeue in-place DingTalk calendar invitation updates through recent-message recovery and bind exact calendar consumer Skills | 2026-09-14 |
