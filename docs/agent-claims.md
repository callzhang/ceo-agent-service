...
| claude-meeting-time-source | app/meeting_alignment_source.py (start/end time agreement only), tests/test_meeting_alignment_source.py, docs/agent-claims.md | Drop the list-vs-info meeting time agreement check and take the info time: DingTalk removed list endTime on 2026-09-18 and the two sources now drift up to 41 minutes, which skipped every meeting that day (Derek: 容差没用的话就没必要存在校验) | active |
