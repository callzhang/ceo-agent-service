...
| --- | --- | --- | --- |
| codex-task-first-main-integration | docs/agent-claims.md (merge claim rows only), tests/test_task_agent.py (resolve main integration conflicts only), tests/test_task_models.py (resolve main integration conflicts only) | Integrate current main `673f4cd5` into `codex/task-first-tasks`; preserve both branches' test intent and claim history while reviewing three content conflicts. No production merge, deploy, or restart. | 2026-09-24 |
...
| codex-task-first-agent-cutover | app/task_models.py, app/task_agent.py, app/task_completion_agent.py, app/task_retrieval.py, app/task_semantic_service.py (optional existing-transaction connection only), app/task_business_resolution.py (optional existing-transaction connection only), app/cli.py (work-summary source dispatch only), app/todo_completion.py (typed completion operation and candidate search-policy producer only), app/follow_up.py (typed follow-up completion operation and search-policy producer only), tests/test_task_models.py, tests/test_task_agent.py, tests/test_task_completion_agent.py, tests/test_task_retrieval.py, tests/test_task_semantic_service.py (atomic batch/connection tests only), tests/test_task_business_resolution.py (atomic proposal/connection tests only), tests/test_cli.py (work-summary source dispatch cases only), tests/test_todo_completion.py (typed completion producer/handler integration only), tests/test_follow_up.py (typed follow-up completion cases only), tests/test_routed_result_privacy.py (Task Agent current contract cases only), tests/test_work_tracking_skill.py (Task-first skill and live-binding assertions only), docs/architecture.md, docs/runtime-mechanism.md, /Users/derek/.agents/skills/ceo-work-tracking/SKILL.md (Task Agent unified lifecycle output protocol and completion operation contract only), docs/agent-claims.md | Replace Task Agent Project-first decisions and legacy Project/TODO writes with one unified TaskAgentDecision contract covering new Task, existing Task transitions, TODO completion, and linked follow-up updates; apply source-typed service handlers to the same Agent decision/result path; add optional existing-transaction connection support so task transitions, relations and proposals apply atomically without nested SQLite connections; preserve run/history handling; Task 7 TODO mirror and deployment explicitly excluded. |
