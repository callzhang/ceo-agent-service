...
| codex-meeting-hr-recipient | app/meeting_alignment.py (_build_meeting_roster_evidence participant identity hydration only), app/meeting_alignment_delivery.py (HR primary-recipient merge only), app/meeting_alignment_agent.py (HR audience target contract only), app/meeting_alignment_models.py (HR audience field descriptions only), tests/test_meeting_alignment.py (identity hydration regression only), tests/test_meeting_alignment_delivery.py (HR merge and HR-group single-delivery regressions only), tests/test_meeting_alignment_agent.py (HR audience prompt contract only), tests/test_meeting_alignment_models.py (HR audience schema descriptions only), docs/runtime-mechanism.md (meeting sensitive-audience paragraph only), docs/architecture.md (meeting audience-routing paragraph only), docs/agent-claims.md | Resolve non-current calendar attendees to stable identities and let an automatically matched HR audience receive shared sensitive meeting content in one message; retain private delivery for specific-person or ambiguous audiences | 2026-09-20 |
