...
| codex-task6-semantic-evidence-dates | app/task_semantic_models.py, app/task_semantic_rules.py, app/task_semantic_service.py, app/store.py (semantic Task schema/date evidence/migration/STORE_SCHEMA_VERSION only), tests/test_task_semantic_rules.py, tests/test_task_semantic_service.py, tests/test_task_semantic_store.py, tests/test_task_business_resolution.py and tests/test_task_attention_projection.py (formal-task fixture helpers only), tests/test_store.py (pinned schema version literal only), docs/task-semantic-storage.md, docs/agent-claims.md | Implement Task 6 source-backed owner, acceptance and typed date contract. claude-todo-sync-skipped shipped in 8d237619 and its stale claim row was cleared after main coordination. No Task Agent caller/API/UI/runtime/live-data cutover or service restart. | 2026-09-23 |
