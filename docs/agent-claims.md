...
| --- | --- | --- | --- |
| codex-task6-acceptance-source-ref-uniqueness | app/task_semantic_service.py (acceptance reply binding only), app/store.py (source reference to formal Task query only), tests/test_task_semantic_service.py (ambiguity regressions only), docs/task-semantic-storage.md (acceptance contract only), docs/agent-claims.md | Reject short owner acceptance when a shared source reference identifies multiple formal Tasks; preserve unique-source replies. No API/UI/retrieval/runtime/live-data change. | 2026-09-23 |
