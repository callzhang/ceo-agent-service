...
| Codex session `runtime-pause-retry` | `app/agent_turn_runner.py`, `tests/test_agent_turn_runner.py` | convert a route-pause race during successor attempt claim into the existing retryable runtime boundary instead of terminal codex_process_failed | 2026-09-12 |
