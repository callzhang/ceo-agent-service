...
| --- | --- | --- | --- |
| claude-consumer-durable-memory | app/agent_contracts.py (DurableMemory + ConsumerAgentResult field), app/agent_wire_contracts.py (consumer wire classes), app/schemas/consumer_agent_result.schema.json, app/store.py (task_memory_write_events + finalize_orchestrated_reply_task memory rows + schema manifests), app/worker.py / app/email_worker.py / app/agent_cron/consumer.py (finalize call sites only), app/task_memory_write.py (new), app/memory_connector_client.py (write arguments), app/agent_cron/commands.py + seeds.py (new command), app/cli.py (new command), app/codex_runner.py (--disable hooks), app/audit_web.py (task memory failed rows only), matching tests, docs/architecture.md, docs/runtime-mechanism.md, docs/superpowers/specs/2026-09-24-consumer-durable-memory-design.md | Derek 2026-09-24：执行 Agent 给出 durable_memories，系统写 Memory；服务 Codex 运行关钩子 | 2026-09-24 |
...
| claude-meeting-body-markdown | app/meeting_alignment_agent.py (final_message format sentence only), tests/test_meeting_alignment_agent.py (format-instruction assertions only), docs/runtime-mechanism.md (only the final_message body-format sub-clause in the follow-up paragraph; title/time/UTC content left to claude-meeting-followup-names-its-meeting) | Derek 2026-09-24 「会议总结消息要用 markdown 结构化、提高可读性和信息密度」，并确认钉钉渲染块级 Markdown。正文契约从『不要 Markdown』改为加粗小节+`- `列表。未改 delivery.py header。 | active |
