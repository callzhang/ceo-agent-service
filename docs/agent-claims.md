...
| --- | --- | --- | --- |
| codex-task-first-contract-repair | app/task_semantic_models.py, app/store.py (business semantic schema and methods only), tests/test_task_semantic_store.py, tests/test_store.py (schema version only), docs/task-semantic-storage.md, docs/agent-claims.md | Take over the partial Task 1 repair in isolated codex/task-first-tasks; complete approved records, relationships, constraints, manifest, and primitive CRUD with red/green evidence; no runtime integration or restart | 2026-09-22 |
