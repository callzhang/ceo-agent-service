...
| --- | --- | --- | --- |
| codex-task6-semantic-acceptance-followup | app/task_semantic_service.py (acceptance source binding, owner reassignment, merged date evidence only), app/store.py (date-evidence copy primitive only), tests/test_task_semantic_service.py (corresponding regressions only), docs/task-semantic-storage.md (existing semantic sections only), docs/agent-claims.md | Repair Task 6 acceptance binding, accepted-owner reassignment, and merged typed-date provenance; coordinated disjoint Store region with retrieval owner; no Task Agent/API/UI/runtime/live data change | 2026-09-23 |
...
| codex-task6-retrieval-grounding | app/task_retrieval.py (semantic retrieval only), app/store.py (semantic cluster membership read API only), tests/test_task_retrieval.py (semantic retrieval tests only), docs/task-semantic-storage.md (retrieval truth only), docs/agent-claims.md | Ground Task 6 retrieval in source signals, per-task relations/links, official Project anchors, and cluster membership. No Task Agent caller/API/UI/runtime/live-data change. | 2026-09-23 |
