...
| codex-oa-skill-binding | app/agent_cron/seeds.py (OA prompt and operation Skill refs only), tests/test_agent_cron_seeds.py (OA seed cases only), docs/architecture.md (OA rule binding only), docs/runtime-mechanism.md (OA rule binding only), docs/agent-claims.md | Derek explicitly authorized direct edit and restart on 2026-09-23 despite overlap with `claude-loops-become-scheduled-tasks`; narrow OA-only scope. Bind all review categories and personal rules to the scheduled OA task, and document/test the binding. | active |
