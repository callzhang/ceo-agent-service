...
| codex-task-first-rules-quality-repair | app/task_semantic_service.py (acceptance/commitment and promotion-owner evidence boundaries only), tests/test_task_semantic_service.py (Task 3 quality regressions only), docs/agent-claims.md | Repair Task 3 quality P1/P2: stop invalid commitment transitions and reconcile promotion owner/missing evidence without losing unrelated gaps. No schema/API/UI/runtime/live-data change. Shipped in `2aa25aba`. | 2026-09-22 |
