...
| claude-meeting-time-source | app/meeting_alignment_source.py (start/end time agreement only), app/meeting_alignment.py (status guard, discovery window, segment dedup), app/store.py (find_meeting_alignment_job_for_segment only), tests/test_meeting_alignment_source.py, tests/test_meeting_alignment.py (those cases only), docs/agent-claims.md | Drop the list-vs-info meeting time agreement check and take the info time: DingTalk removed list endTime on 2026-09-18 and the two sources now drift up to 41 minutes, which skipped every meeting that day (Derek: 容差没用的话就没必要存在校验) | active |
