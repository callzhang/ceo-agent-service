...
| --- | --- | --- | --- |
| codex-meeting-recall-group | app/meeting_alignment.py (group-discovery context only), app/meeting_alignment_agent.py (prompt context only), app/dws_client.py (conversation search only), tests/test_meeting_alignment.py and tests/test_meeting_alignment_agent.py (new group cases only), tests/test_dws_client.py (conversation search case only), docs/runtime-mechanism.md (meeting group-discovery paragraph only), docs/agent-claims.md | Recall fix shipped e94befb6; now replace legacy group-search schema and supply live group candidates before meeting decisions | 2026-09-23 |
