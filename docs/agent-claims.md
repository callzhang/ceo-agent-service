...
| Claude session `self-echo-trigger` | `app/outbound_postfix.py`, `app/store.py`, `app/worker.py`, `tests/test_outbound_postfix.py`, `tests/test_worker.py`, `tests/test_task_store.py` | stop the service reading its own delivered messages back as triggers in the robot chat, and let a history row carry the run's outcome instead of repeating the trigger text | 2026-09-12 |
