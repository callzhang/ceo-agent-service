...
| codex-task-first-rules | app/task_semantic_rules.py (new), app/task_semantic_service.py (rule-derived formality and identity entry points only), tests/test_task_semantic_rules.py (new), tests/test_task_semantic_service.py (rule integration cases only), docs/agent-claims.md | Task 3 of the approved Task-first plan: make promotion and duplicate identity decisions structured and deterministic, without early clustering, schema, API, UI, runtime, or live-data changes. Shipped in `599bd79a`. | 2026-09-22 |
