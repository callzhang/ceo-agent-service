...
| --- | --- | --- | --- |
| Codex subagent `sqlite-error-hardening` | `app/store.py`, `tests/test_store.py`, `CHANGELOG.md` | name and propagate SQLite open/PRAGMA I/O failures; only treat explicit missing-schema errors as uninitialized | 2026-09-14 |
| Codex session `lint-gate-repair` | `app/audit_web.py`, `app/wechat/accessibility.py`, `tests/browser/test_email_unsubscribe_browser.py`, `tests/e2e/test_consumer_audit_live.py`, `tests/e2e/test_email_unsubscribe_audited.py`, `tests/test_agent_turn_store.py`, `tests/test_audit_web.py`, `tests/test_dingtalk_local_sso_confirm.py`, `tests/test_email_folder_classifier_e2e.py`, `tests/test_email_task_adapter.py`, `tests/test_email_unsubscribe.py`, `tests/test_email_worker.py` | repair the currently reported full-repository ruff lint gate without changing business behavior; takes over the stale `consumer-email-minutes-repair` ownership of `tests/test_email_worker.py` under Derek's authorization | 2026-09-14 |
| Codex session `lint-gate-repair` | `app/email_candidate_benchmark.py`, `tests/test_email_candidate_benchmark.py`, `tests/test_workbench_store.py` | repair full `npm test` failures discovered after the lint gate: candidate benchmark input lifecycle contract and stale Workbench schema-version assertions | 2026-09-14 |
