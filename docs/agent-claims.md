...
| --- | --- | --- | --- |
| codex-dingtalk-markdown-send | app/dws_client.py (`build_send_message_command`, idempotent retry/duplicate handling, and `_sanitize_command` only), tests/test_dws_client.py (DingTalk send builder, retry, and sanitizer cases only), tests/test_message_send_architecture.py (raw carrier command-path assertion only), docs/runtime-mechanism.md (DingTalk/WeChat unified outbound suffix section only), docs/agent-claims.md | Send service-prepared DingTalk outbound messages through `chat +messages-send --markdown`, preserving stable targets, idempotency, mentions, and safe diagnostics | 2026-09-21 |
| codex-email-dual-lookback | app/email_connector_config.py, app/email_scheduled_command.py, app/email_classifier_scan.py, app/email_classifier_runtime.py (OnlineClassificationResult review evidence only), app/email_worker.py (model pending-feedback persistence and lookback field only), app/email_store.py (email account lookback columns and v42-to-v43 migration only), app/web_api/email.py (email account lookback response only), frontend/src/api/console.ts (EmailAccount lookback fields only), frontend/src/pages/SettingsPage.tsx, frontend/src/pages/SettingsPage.test.tsx, tests/test_email_connector_config.py, tests/test_email_scheduled_command.py, tests/test_email_classifier_scan_model.py, tests/test_email_classifier_runtime.py, tests/test_email_store.py (v43/lookback cases only), tests/test_email_web_api.py (account lookback case only), docs/architecture.md, docs/runtime-mechanism.md, docs/superpowers/specs/2026-09-21-email-dual-lookback-classification-design.md, docs/superpowers/plans/2026-09-21-email-dual-lookback-classification.md, docs/agent-claims.md | Add independent Agent/model lookback windows; scheduled model history auto-files accepted predictions and sends rejected predictions to existing pending-feedback review without Agent fallback. Builds on v42 and limits overlaps with existing claims to the named fields/hunks. | 2026-09-21 |
