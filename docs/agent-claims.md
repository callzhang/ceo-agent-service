...
| codex-oa-skill-binding | app/agent_cron/seeds.py (OA migration compatibility only), tests/test_agent_cron_seeds.py (OA migration case only), docs/agent-claims.md | Derek explicitly authorized direct edit and restart on 2026-09-23 despite overlap with `claude-loops-become-scheduled-tasks`; recognize and migrate the prior default OA prompt/Skill refs while preserving user-customized config. | active |
