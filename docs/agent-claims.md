...
| claude-minutes-sync-batch-cap | app/minutes_sync.py (`max_new_items` removal only), app/cli.py (`sync_minutes_once_command` and its two call sites only), tests/test_minutes_sync.py, tests/test_cli.py (the two `sync-minutes-once` cases that asserted the cap only), CHANGELOG.md, docs/agent-claims.md | The daily AI 听记 pass ran with `CEO_MAX_BATCHES=4` as a per-pass item cap, so it archived only the four newest minutes a day and the rest fell below the newest listing page and were never offered again; drop the cap and backfill the missing minutes | 2026-09-17 |
