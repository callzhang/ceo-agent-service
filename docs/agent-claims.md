...
| codex-oa-skill-only-runtime-source | app/agent_cron/seeds.py (OA task prompt and migration constants only), app/consumer_agent.py (OA instructions paragraph only), tests/test_agent_cron_seeds.py (OA assertions only), tests/test_consumer_agent.py (OA prompt assertions only), docs/architecture.md (OA source paragraph only), docs/runtime-mechanism.md (OA source paragraph only), docs/agent-claims.md | Derek, 2026-09-23: `钉钉审批审阅原则.md` is reference material, not an Agent runtime source. Use the generic OA Skill plus applicable Stardust business Skills; include the cloud-resource review Skill in default routing and keep action coverage accurate. Deployed: 07b82de1 + 0cec6e2f; verified PID 44808, task 4 v9 with seven refs, healthy queues and healthz. | done |
...
| codex-oa-result-schema-fixture | tests/test_agent_turn_store.py (needs_human slot fixture only), docs/agent-claims.md | Refresh the legacy needs_human wire fixture to include required decision evidence and use the current authorization/error contract; no runtime behavior change | 2026-09-23 |
