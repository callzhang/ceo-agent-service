...
会议总结的业务群候选由实时群搜索取得。对于完整日历名册，服务进一步读取候选群成员，向决策 Agent 提供可核验参会人的覆盖人数和群规模；同名会议曾至少两次成功投递到同一群时，还提供该历史承接证据。历史投递只有在本次实时成员覆盖仍达到门槛时才成为强候选。存在强候选时，业务会议不能以“仅有群名线索”为由回退给组织者私聊；目标校验会要求重新选择群。群覆盖不等于议题授权，Agent 仍须核对群受众与本次完整议题；此前已发送的总结不会因路由修复自动重发。
