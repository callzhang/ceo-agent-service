...
**定时完成检查已停用**（Derek 2026-09-25：「不需要定期检查未完成任务，只需要定期扫描新信息并更新相应的 task」）：`check-follow-up-completions` 不再入队任何 Work Item，维护循环与每日维护中的调用因此都不产生完成检查。Task 只在新信息到来时更新——消息、会议、钉钉待办状态由各来源扫描交给 Task Agent。停用前，定时检查把每条开放 TODO 连同其快照重发给 Task Agent，16 次中 15 次一次都没检索就以自身编号为来源而失败。关闭来源上的 Task 后，Attention 成员在领域提交后更新；同一关注项中仍开放的兄弟 Task 继续保留。
