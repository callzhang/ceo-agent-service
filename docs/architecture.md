...
一条总结，不能返回 `no_action`。业务群发现或投递失败是可重试或最终失败的执行结果，服务绝不私信会议创建人或参会人。
...
权威会话信息证明不可发送时，服务重试群验证或使用 Agent 的下一证据候选群；不会回退到任何人。
会议没有实质分歧时，仍发送简短的已确认事项和下一步；听记缺失或已删除时明确记录为失败，
不会伪装成“无需动作”。
