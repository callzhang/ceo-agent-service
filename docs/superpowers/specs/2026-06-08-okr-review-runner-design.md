...
OA 审批也必须迁移到同一机制。OA runner 不再默认独立开 session。
...
OA runner 也迁移到该 loader，不再在 OA 模块里单独读 skill。
...
5. 将 OA runner 迁移到 `StructuredCodexRunner`，并让 OA handler 读取 `system_actions` 执行 DWS。
