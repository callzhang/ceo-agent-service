...
## 2026-09-01 新随机只读结构实验

为继续验证随机抓取的链路成本，同时避免把未经明确授权的邮件正文片段输出到实验
记录，本轮使用固定种子 `20260905` 从当前 INBOX 的 `2299` 个 UID 中随机抽取
`100` 封，并通过生产 `ImapReadonlyAdapter` 读取有限纯文本部分。结果为 `100/100`
读取成功，UIDVALIDITY 为 `2`；正文长度 P50/P95 约为 `953/5714` 字符，空正文
`4` 封。

样本中有 `36` 封包含附件 metadata，共 `86` 个附件；metadata 汇总的声明大小为
约 `52.9 MB`。本轮没有下载、打开、解析或推断任何附件内容，也没有执行
`STORE`、移动、删除、`EXPUNGE`、SMTP 发信或其他邮箱写操作。输出仅保留聚合统计，
没有保存或展示发件人、主题、正文、URL 或 UID。

本轮样本的 `List-Unsubscribe`、`List-Unsubscribe-Post` 和 `Auto-Submitted` 均为
`0`。这可以作为当前快照的读取与成本证据，但不能推断未来邮件没有退订入口，也
