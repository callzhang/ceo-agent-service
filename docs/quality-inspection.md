...
当几个可逆选项都不改变既有事项时，Agent 应选择不为他人新增工作或交付物的最小路径。Audit 应把这项
低后果策略视为已授权的判断标准，不能要求历史对话中已经有相同措辞；只在实时事实冲突或动作改变范围、
时间、owner、业务含义、预算、审批、敏感目标或外部承诺时退回。
...
若未知 Audit run 已持久化每个批准动作的受控完成事件，且每个动作都有同目标的后续读回，
服务可直接确认终态，不再要求恢复模型重复输出读回摘要。操作身份、命令摘要、目标和读回
关系仍必须全部匹配；缺少任一证据时继续只读 reconciliation，绝不据此重放写入。

读回关系由 `config/mcp-tool-effects.json` 的命令契约声明。新增可恢复操作时必须同时登记
其受控写命令和同目标读取命令，并添加正反向测试；不得以某个具体业务任务绕开匹配。
