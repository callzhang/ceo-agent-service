...
- 没有明确问题。
- 同一条消息里没有 `麻烦看一下` / `请 review` / `给 comments` 这类请求。
- 没有要求审阅的普通钉钉文件。
...
- 如果包含审批/OA 链接特征，进入 agent 并套用 OA 审阅原则。
- 如果只是状态通知、没有问题或处理请求，未来可以考虑 `skip_before_agent`。
- 如果要求 Derek 处理，进入 agent 或交给本人。
...
| 自动同步通知 | agent/model 或暂无样本 | 否 | 先收集样本 |
| 文件状态通知 | agent/model 或暂无样本 | 否 | 先收集样本 |
| 项目/流程状态通知 | agent/model | 仅有 OA 链接例外 | 有样本后再加 |
