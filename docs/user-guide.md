...
传给 Agent。如果旧任务中已有当前不支持的附件，界面会阻止发送，必须先移除该附件。Agent 生成的报告、
图片或其他文件会显示为产物，可从当前任务下载。浏览器响应不暴露本机存储路径或 provider 会话标识，
附件、产物和任务状态保存在该用户自己的本地目录与 SQLite 中；仍应只上传当前管理者有权处理的资料。
