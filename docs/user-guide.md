...
用猜测补齐。命令、参数、工具结果、对话和错误里的绝对本地路径都会原样显示，不再替换成 `[local path]`；只有
令牌、密码、Cookie、认证头等实际凭证值会在保留字段名的同时替换为 `[REDACTED]`。任务结束时仍未收到完成
事件的工具会标记为“已中止”；被用户停止且没有退出码的命令也显示“已中止”，不会误报成执行失败。真正返回
非零退出码的命令仍如实显示失败和退出码。
...
工具透明化只改变可观察性，不改变授权边界：通过 Codex 命令审阅器的只读命令和 MCP 调用会直接展示并执行；需要审阅的写操作仍必须
经过确认卡，确认的是卡片所列规范化目标和参数，而不是一个隐藏的替代命令。若底层 Codex 的命令审阅器拒绝
某条命令且没有返回退出码或诊断输出，卡片会明确标明这是 Provider 边界，并保留原始事件；它不会伪造成功或
猜测一个不存在的退出码。
