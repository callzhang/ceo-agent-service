...
中间是当前任务的对话和执行进度，右侧是本轮工具、确认项、产物和统计。新建任务时先写清目标并选择
runtime；再次打开已有任务，可以在同一上下文中继续提问或追加工作，不需要重新粘贴之前的对话。

runtime 列表会展示当前适配器的能力，例如能否恢复会话、流式输出、显示结构化工具、接收图片、选择模型、
使用 MCP、停止和故障恢复。首个生产版本只使用 **Codex** 执行真实任务。Claude 和 Pi 已用同一运行契约
验证兼容性，但目前没有生产适配器，不能把契约测试理解为已经可以在页面中真实运行它们。
...
普通文件可作为本机附件加入任务。图片只有在所选 runtime 声明支持图片输入时才会按图片传入；否则它只会
作为普通附件，不能假设 Agent 已经看懂画面。Agent 生成的报告、图片或其他文件会显示为产物，可从当前任务
下载。浏览器响应不暴露本机存储路径或 provider 会话标识，附件、产物和任务状态保存在该用户自己的本地目录
与 SQLite 中；仍应只上传当前管理者有权处理的资料。
