...
会议投递先按内容确定范围，日历仅用于确认参会名单。业务内容必须由 Agent 使用业务承接证据
搜索、排序并自行选择最强的可发送群；多个合理群由 Agent 决定，不由服务改选。权威会话信息
证明所选群不可发送、且会议上下文已有稳定日历组织者身份时，服务把同一总结私聊给组织者；
服务不按姓名搜索或猜测组织者。逐字稿发言人不是完整参会名单证据。
只有个人、非业务内容，且完整日历名单证明 Derek 与另一位参会人两人参会时，才允许 direct 到
该另一位参会人。所选业务群被权威会话信息证明不可发送时，服务重试群验证、使用下一证据候选群，
或使用稳定日历组织者 direct fallback；不回退为任意参会人私信。
