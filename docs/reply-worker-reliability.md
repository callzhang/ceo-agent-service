...
浏览器通知按 attempt ID 固定，同一 attempt 的更新不会制造并列弹窗。通知正文直接显示事项、
状态、原因和下一步，点击可打开该 attempt 的审计详情；它不能退化为只有“X 项问题待处理”的
计数提示。普通用户页面展示业务类型、对话、问题、候选/结果和恢复条件；A/B 内部标签、session
