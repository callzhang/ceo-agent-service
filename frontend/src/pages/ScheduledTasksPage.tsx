      ...
      <section className="scheduled-task-detail" aria-label={creating ? "新建定时任务" : "定时任务编辑器"}>
        {(creating || selected) ? <>
          ...
          <form className="scheduled-task-form" onSubmit={(event) => { event.preventDefault(); void save(); }}>
            ...
            {commandTask ? <>
            <section className="scheduled-task-system-prompt" aria-labelledby="scheduled-task-system-prompt-title"><div className="scheduled-task-section-heading"><h3 id="scheduled-task-system-prompt-title">Consumer Agent 系统提示词</h3><span>系统管理 · 只读</span></div><pre>你正在处理由 Trigger 产生的真实业务任务。只处理当前业务对象，遵守对应 Skill、Runtime 和审计协议；不得把 Trigger 检查动作当作用户请求，不得重复消费或重复发送。</pre><small>Trigger 发现真实消息后，Consumer Agent 使用这段系统提示词。</small></section>
            <div className="scheduled-task-prompt-field"><label htmlFor="scheduled-task-consumer-custom-prompt">Consumer Agent 自定义描述</label><textarea id="scheduled-task-consumer-custom-prompt" aria-label="Consumer Agent 自定义描述" rows={5} defaultValue="重点关注需要 Derek 决策、确认、承诺或跟进的消息；普通通知和已完成事项优先 no_action。" placeholder="补充这个 workflow 的业务重点" /></div>
            <div className="scheduled-task-skill-block"><div className="scheduled-task-section-heading"><h3>Consumer Agent Skills</h3><span>结构化配置 · Trigger 发现真实消息后使用</span></div><div className="scheduled-task-skill-chips"><button type="button" className="scheduled-task-skill-chip">ceo-message-triage <small>Managed</small><b aria-hidden="true">×</b></button><button type="button" className="scheduled-task-skill-chip">dingtalk-chat <small>Operation</small><b aria-hidden="true">×</b></button><button type="button" className="secondary-button">+ 添加 Skill</button></div></div>
            <div className="scheduled-task-form-row"><label><span>Consumer Agent Runtime</span><select aria-label="Consumer Agent Runtime" defaultValue="codex_api"><option>codex_api</option><option>codex_oauth</option></select></label></div>
            </> : <>
            ...
            <section className="scheduled-task-system-prompt" aria-labelledby="scheduled-task-system-prompt-title"><div className="scheduled-task-section-heading"><h3 id="scheduled-task-system-prompt-title">Consumer Agent 系统提示词</h3><span>系统管理 · 只读</span></div><pre>你正在处理由 Trigger 产生的真实业务任务。只处理当前业务对象，遵守对应 Skill、Runtime 和审计协议；不得把 Trigger 检查动作当作用户请求，不得重复消费或重复发送。</pre><small>这段提示词由 CEO Agent Service 统一维护，用户不能修改。</small></section>
            <div className="scheduled-task-prompt-field"><label htmlFor="scheduled-task-prompt">Consumer Agent 自定义描述</label><textarea id="scheduled-task-prompt" aria-label="任务描述" rows={7} value={draft.prompt} onChange={(event) => { updatePrompt(event.target.value); setSkillMenuOpen(activeSkillQuery(event.target.value) !== null); }} placeholder="补充这个 workflow 的业务重点；输入 $ 引用 Skill" />
