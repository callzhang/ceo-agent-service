      ...
      <section className="scheduled-task-detail" aria-label={creating ? "新建定时任务" : "定时任务编辑器"}>
        {(creating || selected) ? <>
          ...
          <form className="scheduled-task-form" onSubmit={(event) => { event.preventDefault(); void save(); }}>
            ...
            <div className="scheduled-task-skill-block"><div className="scheduled-task-section-heading"><h3>从提示词提取的 Skills</h3><span>只读 · 确定性解析</span></div><div className="scheduled-task-skill-chips"><span className="scheduled-task-skill-chip">$ceo-message-triage <small>Managed</small></span><span className="scheduled-task-skill-chip">$dingtalk-chat <small>Operation</small></span></div></div>
