...
export function HistoryPage() {
  ...
  useEffect(() => {
    ...
    setState("loading");
    setError("");
    listHistory({ q: query, status, object_type: objectType, page, page_size: pageSize, chart_range: chartRange }, controller.signal).then((result) => { if (generation !== requestGeneration.current) return; setRows(result.items); setTotalCount(result.meta.total || 0); setChart(result.chart); setSnapshot(result.meta.snapshot_at); setPendingStatus(null); setState("ready"); }).catch((reason: unknown) => { if (controller.signal.aborted || generation !== requestGeneration.current) return; setError(reason instanceof Error ? reason.message : "加载失败"); setPendingStatus(null); setState("error"); });
    return () => controller.abort();
  }, [query, status, objectType, page, pageSize, chartRange]);
