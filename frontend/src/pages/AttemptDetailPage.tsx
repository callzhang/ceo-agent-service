...
import { getAttemptDetail, type AttemptAgentSession, type AttemptConsumerResult, type AttemptDetail, type AttemptEmail, type AttemptMetadata, type AttemptRuntimeEntry, type AttemptToolUse } from "../api/attempts";
...
function MetadataGrid({ rows, consumerResult }: { rows: AttemptMetadata[]; consumerResult?: AttemptConsumerResult }) {
  return <section className="console-card attempt-metadata-card"><div className="attempt-metadata-grid">{rows.map((row) => <div className="attempt-metadata-item" key={row.label}><span>{row.label}</span><strong>{row.value || "未记录"}</strong></div>)}{consumerResult && <ConsumerResult result={consumerResult} />}</div></section>;
}
...
function EmailContext({ email }: { email: AttemptEmail | null }) {
  ...
  return <section className="console-card attempt-email-card"><div className="execution-detail-list-header"><div><h2>关联邮件</h2><p>这条 Attempt 处理的邮件，以及它为这封邮件留下的回执。</p></div></div><dl className="attempt-email-grid">{rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>{receipt && <div className="attempt-email-receipt"><h3>退订回执</h3><dl className="attempt-email-grid"><div><dt>结果</dt><dd>{receipt.outcome}</dd></div><div><dt>依据</dt><dd>{receipt.evidence}</dd></div><div><dt>开始</dt><dd>{receipt.started_at}</dd></div><div><dt>结束</dt><dd>{receipt.completed_at}</dd></div></dl>{receipt.result_text && <p className="attempt-email-observation">{receipt.result_text}</p>}{receipt.steps.length > 0 && <ol className="attempt-email-steps">{receipt.steps.map((step) => <li key={step.sequence}><strong>{step.operation}</strong><span>{step.state}</span></li>)}</ol>}</div>}</section>;
}
...
function FeedbackEvents({ events }: { events: AttemptDetail["feedback"]["events"] }) {
  ...
  return <section className="feedback-iteration-section"><h3>对方反馈</h3><div className="attempt-feedback-event-list">{events.map((event, index) => <article key={`${event.received_at}-${index}`}><dl className="feedback-iteration-values"><div><dt>对方反馈（1-5星）</dt><dd>{event.rating_stars || event.rating_label || "未提供"}</dd></div><div><dt>对方反馈内容</dt><dd className="feedback-iteration-text">{event.comment || "未填写内容"}</dd></div></dl>{(event.source || event.received_at) && <div className="attempt-feedback-event-meta">{event.source && <span>来源：{event.source}</span>}{event.received_at && <time>{event.received_at}</time>}</div>}</article>)}</div></section>;
}
