...
type SettingsSection = "status" | "info" | "configuration" | "agent-runtime" | "prompts" | "work-profile" | "connectors" | "audit-rules" | "skills" | "mcp" | "attention";
type WorkProfileView = "source" | "injection";
...
const sections: Array<[SettingsSection, string]> = [
  ...
  ["prompts", "Prompts"], ["work-profile", "Work Profile"], ["connectors", "Connectors"], ["audit-rules", "Audit Rules"], ["skills", "Skills"], ["mcp", "MCP"], ["attention", "Attention"],
];
...
function WorkProfilePanel({ payload, draft, setDraft, view, saveState, saveError }: { payload: RecordValue; draft: RecordValue; setDraft: (value: RecordValue) => void; view: WorkProfileView; saveState: "idle" | "saving" | "saved" | "error"; saveError: string }) {
  const profile = rawValue(draft, payload, "profile");
  const path = displayValue(fieldsOf(payload).path);
  const injection = displayValue(record(payload.preview).injection);
  return <SettingsCard>
    <div className="settings-card-heading"><div><h2>Work Profile</h2><p className="muted">工作人格独立于 Developer Prompt 和 User Prompt。服务会在每次新的 Consumer 或 Audit 运行时，将它追加到基础 Developer Prompt。</p></div><span className="settings-path">{path || "runtime profile"}</span></div>
    <div className="settings-pill-row" role="tablist" aria-label="Work Profile view"><Link role="tab" aria-selected={view === "source"} className={view === "source" ? "active" : ""} to="/settings?tab=work-profile&view=source">Profile source</Link><Link role="tab" aria-selected={view === "injection"} className={view === "injection" ? "active" : ""} to="/settings?tab=work-profile&view=injection">Runtime injection</Link></div>
    <div role="tabpanel" aria-label={view === "source" ? "Profile source" : "Runtime injection"}>{view === "source" ? <form onSubmit={(event) => event.preventDefault()}><TokenEditor id="work-profile" label="Distilled work profile" value={profile} onChange={(next) => setDraft({ ...draft, profile: next })} rows={22} /><p className="muted">保存后会作用于下一次新建的 Consumer 或 Audit 运行；已开始的运行保留其启动时的上下文。</p><SaveBar state={saveState} error={saveError} /></form> : <><p className="muted">这是服务实际追加到基础 Developer Prompt 的内容，用于核对注入边界。</p><pre className="prompt-preview">{injection || "未提供运行时注入内容"}</pre></>}</div>
  </SettingsCard>;
}
...
function SettingsContent({ section, payload, draft, setDraft, prompt, view, workProfileView, connector, auditRule, saveState, saveError, onAttentionCountChange }: { section: SettingsSection; payload: RecordValue; draft: RecordValue; setDraft: (value: RecordValue) => void; prompt: "developer" | "user"; view: "template" | "preview"; workProfileView: WorkProfileView; connector: string; auditRule: "template" | "consumer" | "audit"; saveState: "idle" | "saving" | "saved" | "error"; saveError: string; onAttentionCountChange: (count: number) => void }) {
  ...
  if (section === "work-profile") return <WorkProfilePanel payload={payload} draft={draft} setDraft={setDraft} view={workProfileView} saveState={saveState} saveError={saveError} />;
...
export function SettingsPage() {
  ...
  const workProfileView: WorkProfileView = params.get("view") === "injection" ? "injection" : "source";
