...
export function ModelTraining({learning,configs,reload,runtimeVerified,onRuntimeUnverified,onBusy}: {learning:EmailLearningEvidence;configs:EmailCategoryConfig[];reload:RefreshLearning;runtimeVerified:boolean;onRuntimeUnverified:()=>void;onBusy:(busy:boolean)=>void}) {
  ...
  return <section className="console-card email-training">
    ...
    {!models.length&&!learning.models?.length?<p>暂无训练版本。</p>:<div className="responsive-table-wrap"><table className="settings-table email-model-table" aria-label="模型版本"><thead><tr><th>完整模型名与版本</th><th>状态</th><th>训练时间</th><th>样本数</th><th>Macro F1</th><th>端到端 P95</th><th>完整性</th><th>详情</th></tr></thead><tbody>
      ...
      {learning.models?.filter(model=>!models.some(item=>item.model_id===model.model_id)).map(model=><tr key={model.model_id}><td>{model.model_id}</td><td>{statusLabel(model.status)}</td><td>{localTime(model.trained_at)}</td><td>{model.sample_count}</td><td>{measured(model.macro_f1)}</td><td>{measured(model.prediction_latency_p95_ms," ms")}</td><td>{integrityLabel(model.integrity_status,model.integrity_error)}</td><td><button className="compact-button" aria-label={"查看 "+model.model_id} onClick={()=>{setSelected("");setDetail(null);setLegacy(model);}}>查看</button></td></tr>)}
    </tbody></table></div>}
    ...
    {legacy&&<EmailDrawer title="模型版本证据" onClose={()=>setLegacy(null)}><div className="email-drawer-content"><h3>{legacy.model_id}</h3><p>模型家族：{legacy.model_family || "未提供"} · 状态：{statusLabel(legacy.status)}</p><p>Accuracy {measured(legacy.accuracy)} · Macro F1 {measured(legacy.macro_f1)} · P95 {measured(legacy.prediction_latency_p95_ms," ms")}</p><p>样本 {legacy.sample_count}（新增 {legacy.new_sample_count}）· 训练：{localTime(legacy.training_started_at)} → {localTime(legacy.training_finished_at)}</p><p className="muted">候选版本不会自动成为新邮件主模型；只有通过晋升检查并由用户切换后才会生效。</p><details><summary>原始模型证据</summary><pre>{JSON.stringify(legacy,null,2)}</pre></details></div></EmailDrawer>}
  </section>;
