...
import { ChevronLeft, ChevronRight, Maximize2, Minimize2, Star, X } from "lucide-react";
...
export function EmailReadingPanel(props: Props) {
  ...
  const important = provider?.important ?? item?.important;
    ...
    {props.error ? <p role="alert" className="email-reading-empty">{props.error} <button onClick={props.onRetry}>重试正文</button></p> : !item ? <p role="status" className="email-reading-empty">正在加载邮件正文…</p> : <>
      <header className="email-reading-header">
        ...
        <div className="email-reading-actions">
          ...
          <span className="email-important-state" title="邮箱当前的 Star / Flag 状态"><Star size={15} fill={important ? "currentColor" : "none"}/>{important == null ? "重要状态未知" : important ? "重要" : "未标记重要"}</span>
        </div>
      ...
      {tab === "body" ? <div role="tabpanel" id="email-reading-body" aria-labelledby="email-reading-tab-body" className="email-reading-body">
        ...
        {provider && <section aria-label="邮箱观察事实" className="email-provider-state"><h3>邮箱当前状态</h3><p>文件夹：{String(provider.provider_folder_name || provider.category_key || "未知")} · {provider.important == null ? "重要状态未知" : provider.important ? "已标记重要" : "未标记重要"}</p></section>}
