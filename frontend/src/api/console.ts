...
function emailHealth(value: unknown): boolean {
  ...
  const optionalEntryFields = ["error_code", "error_stage", "error_type", "error_fields", "failures", "persisted_count", "task_count", "isolated_count", "superseded_count", "unresolved_count"];
  const validEntry = (value: unknown) => {
    ...
    return entry !== null && strings(entry, ["scope", "status", "updated_at"])
      && Object.entries(entry).every(([key, item]) => {
        ...
        if (key === "error_code" || key === "error_stage" || key === "error_type" || key === "error_fields") return item === null || typeof item === "string";
